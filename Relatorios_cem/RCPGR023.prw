...
Impressão de Transferência - Falecido
@author TOTVS
@since 17/08/2016
@version P12
@param nulo
...
/****************************************/
User Function RCPGR023(cContr,cCodTransf)
/****************************************/        
...
If U19->(DbSeek(xFilial("U19")+cCodTransf))
	...
	cTexto := "Eu, " + cNomeAtu + " brasileiro(a), Residente na "	
	cTexto += cEndAtu + ", " 
	cTexto += "portador do RG: " + cRgAtu + ", e do CPF: " + cCpfAtu + ", " 
	cTexto += "solicito a empresa " + AllTrim(cNomeFant) + ", a efetuar a " 
	cTexto += "transferência do Cessão de Direito de Uso Perpétuo " 
	
	If Len(aEndJz) > 0
		cEndJz := "nº " + cContr + " Quadra " + aEndJz[1][1] + " Módulo " + aEndJz[1][2] + " Jazigo " + aEndJz[1][3] + IIF(Len(aEndJz) > 1," e " + aEndJz[2][3]," ")
		cTexto += " " + cEndJz + " localizado no "  
	Endif

	cTexto += cNomeFant + " para a minha titularidade pelo motivo de "  
	cTexto +=  "falecimento do Sr(a)." + cNomeAnt + ", permanecendo "   
	cTexto +=  "o mesmo contrato original."  
	...
	oRel:Say(nLin,300,"Firmo o presente TERMO DE TRANSFERÊNCIA em 2 (duas) vias de igual teor.",oFont12I)
