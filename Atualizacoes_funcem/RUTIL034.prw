...
Static Function TelaFiltro()
	...
	TGroup():Create(oDlgFil,50,2,143,350,' Adicionar arquivo/extrato da operadora ',,,.T.)
	...
	TGroup():Create(oDlgFil,65,260,100,345,'',,,.T.)
	...
	TSay():New( 106,10,{|| "Dt.Vencto. De" }, oDlgFil,,,,,,.T.,CLR_BLACK,,50,9 )
	oDtVenc1 := TGet():New( 104, 55, {|u| iif( PCount()==0,dDtVenc1,dDtVenc1:= u) },oDlgFil,60,9,,/*bValid*/,,,,.F.,,.T.,,.F.,{|| .T.},.F.,.F.,/*bChange*/,.F.,.F.,,"E1_EMISSAO",,,,.T.,.F.)

	TSay():New( 106,125,{|| "Dt.Vencto. Até" }, oDlgFil,,,,,,.T.,CLR_BLACK,,50,9 )
	oDtVenc2 := TGet():New( 104, 166, {|u| iif( PCount()==0,dDtVenc2,dDtVenc2:= u) },oDlgFil,60,9,,/*bValid*/,,,,.F.,,.T.,,.F.,{|| .T.},.F.,.F.,/*bChange*/,.F.,.F.,,"E1_EMISSAO",,,,.T.,.F.)

	TSay():New( 119,10,{|| "Bandeira" }, oDlgFil,,,,,,.T.,CLR_BLACK,,50,9 )
	oBandeira := TComboBox():New(117, 55, {|u| If(PCount()>0,cBandeira:=u,cBandeira)}, {""} , 160, 012, oDlgFil, Nil,{|| .T./*bChange*/ },/*bValid*/,,,.T.,,Nil,Nil,{|| .T. } )
	...
	//Criado ponto de entrada para criar botao de ajustar arquivo. 
	//A pedido da Nutriza, que o layout em CSV vem com aspas e precisou remover essas aspas.
	...
	TGroup():Create(oDlgFil,146,2,265,350,' Arquivos selecionados ',,,.T.)

	oMSNewGeFil := fMSNewGeFil(oDlgFil)
	oMSNewGeFil:oBrowse:bHeaderClick := {|oBrw1,nCol| if(nCol > 0, UOrdGrid(@oMSNewGeFil, @nCol), )}
...
Static Function fMSNewGeFil(oComp)
	...
	Aadd(aHeaderFil,{ 'Dt.Vencto De'	,'VENCTODE'	,''		,TamSx3("E1_VENCTO")[1],0,'','€€€€€€€€€€€€€€','D','','','',''})
	aadd(aMSEmptyFil, stod(""))

	Aadd(aHeaderFil,{ 'Dt.Vencto Até'	,'VENCTOATE'	,''		,TamSx3("E1_VENCTO")[1],0,'','€€€€€€€€€€€€€€','D','','','',''})
	aadd(aMSEmptyFil, stod(""))

	aadd(aMSEmptyFil, .F.) //deleted
	aAdd(aDadosFil, aClone(aMSEmptyFil))

Return MsNewGetDados():New( 155,05,260,347,,cTrue, cTrue,, aAlterFields,, 99, cTrue, "", cTrue, oComp, aHeaderFil, aDadosFil)
...
Static Function DoAddArq()
	...
	if !empty(dtos(dDtEmis1)) .or. !empty(dtos(dDtEmis2))
		if dDtEmis1 > dDtEmis2 .AND. !empty(dDtEmis2)
			MsgInfo("Datas de emissão não preenchidas corretamente.","Atençao")
			return
		endif
	endif
	if !empty(dtos(dDtVenc1)) .or. !empty(dtos(dDtVenc2))
		if dDtVenc1 > dDtVenc2 .AND. !empty(dDtVenc2)
			MsgInfo("Datas de emissão não preenchidas corretamente.","Atençao")
			return
		endif
	endif
	...
	aAdd(aDadosFil, {cLayout, cDesLay, cArquivo, dDtEmis1, dDtEmis2, cBandeira, SubStr(oBandeira:aItems[oBandeira:nAt],5), iif(lConsCab,"LBOK","LBNO"), dDtVenc1, dDtVenc2, .F./*deleted*/})
...
Static Function DoValidFil()
	...
	if lRet
		...
		If Select("QRYSAE") > 0
			QRYSAE->(DbCloseArea())
		Endif
		cQry := " SELECT AE_COD, AE_ADMCART, AE_REDEAUT "
		...
		if QRYSAE->(!Eof())
			...
			while QRYSAE->(!Eof())
				...
				aadd(aBandOper[nPosX][2], {QRYSAE->AE_COD,QRYSAE->AE_ADMCART} )
...
Static Function TelaFatura()
	...
	Local nPosVct1 := aScan(aHeaderFil,{|x| AllTrim(x[2])=="VENCTODE"})
	Local nPosVct2 := aScan(aHeaderFil,{|x| AllTrim(x[2])=="VENCTOATE"})
	...
	Private oDlg062
	Private oTFolder
	Private aTFolder := {{},{},{}} //{{cod operadora},{titulo da aba},{objeto classe aba}}
	...
	for nX:= 1 to len(aTFolder[1])-1 //codigos dos layouts
		...
		for nY := 1 to len(aDadosFil)
			if aDadosFil[nY][1] == aTFolder[1][nX] //se arquivo for deste layout
				aadd(aFiles, {;
					aDadosFil[nY][nPosArq], ; //arquivo extrato
					aDadosFil[nY][nPosDt1], ; //data emissao 1
					aDadosFil[nY][nPosDt2], ; //data emissao 2
					aDadosFil[nY][nPosCab]=="LBOK", ; //considera cabeçalho
					aDadosFil[nY][nPosBan], ; //Bandeira
					aDadosFil[nY][nPosNBa], ;
					aDadosFil[nY][nPosVct1], ; //data vencto 1
					aDadosFil[nY][nPosVct2] ; //data vencto 2
				})
			endif
		next nY
...
METHOD New(_cLayout, _aFiles, _oDlgAba, _oDlgInc ) CLASS UT019ABA
	...
	if U98->(DbSeek(xFilial("U98")+_cLayout ))
		...
		::aFiles	:= _aFiles //array dos arquivos {{_cArquivo, _dDtEmis1, _dDtEmis2, _lConsCab, _cBandeira, _cNmBand, _dDtVenc1, _dDtVenc2}...}
...
METHOD DoNewGetPri(oPnl) CLASS UT019ABA
	...
	aHeadTmp := UAHEADER("E1_EMISSAO")
	aHeadTmp[1] := "Dt. Venda"
	aadd(::aHeader, aClone(aHeadTmp) )
	aAdd(::aEmptyLin, STOD(""))

	aHeadTmp := UAHEADER("E1_VLRREAL")
	...
	aHeadTmp := UAHEADER("AE_TAXA")
	aHeadTmp[1] += " (%)"
	aadd(::aHeader, aClone(aHeadTmp) )
	aAdd(::aEmptyLin, 0 )

	aHeadTmp := UAHEADER("E1_VALOR")
	...
	::oGetDados:oBrowse:bHeaderClick := {|oBrw,nCol,aDim| if(::oGetDados:oBrowse:nColPos<>111 .and. nCol == 1,(::DoMarcaTodos(::oGetDados, 2, "BR_VERDE"),oBrw:SetFocus()), if(nCol > 1, UOrdGrid(@::oGetDados, @nCol), ))}
...
METHOD DoAddPnlTot(oPnl) CLASS UT019ABA
	...
	TSay():New( 05, 05, {|| "Qtd Registros Operadora:" }, oPnl,,,,,,.T.,CLR_BLACK,,150,9 )
	(aadd(::aObjTotaliz, Nil) , nObj++)
	@ 05, 50 SAY ::aObjTotaliz[nObj] VAR ::nQtdGer SIZE 50, 9 OF oPnl RIGHT COLOR CLR_BLUE PIXEL

	TSay():New( 15, 05, {|| "Qtd Registros Selecionados:" }, oPnl,,,,,,.T.,CLR_BLACK,,150,9 )
	(aadd(::aObjTotaliz, Nil) , nObj++)
	@ 15, 50 SAY ::aObjTotaliz[nObj] VAR ::nQtdSel SIZE 50, 9 OF oPnl RIGHT COLOR CLR_BLUE PIXEL

	TSay():New( 25, 05, {|| "Qtd Registros Inconsistentes:" }, oPnl,,,,,,.T.,CLR_BLACK,,150,9 )
	(aadd(::aObjTotaliz, Nil) , nObj++)
	@ 25, 50 SAY ::aObjTotaliz[nObj] VAR ::nQtdInc SIZE 50, 9 OF oPnl RIGHT COLOR CLR_BLUE PIXEL


	TSay():New( 05, nPxlRight*0.33, {|| "Valor Total Títulos Operadora:" }, oPnl,,,,,,.T.,CLR_BLACK,,150,9 )
	(aadd(::aObjTotaliz, Nil) , nObj++)
	@ 05, (nPxlRight*0.33)+75 SAY ::aObjTotaliz[nObj] VAR ::nVlTotE1Ger SIZE 50, 9 OF oPnl PICTURE PesqPict("SE1","E1_VALOR") RIGHT COLOR CLR_BLUE PIXEL

	TSay():New( 15, nPxlRight*0.33, {|| "Valor Total Títulos Selecionados:" }, oPnl,,,,,,.T.,CLR_BLACK,,150,9 )
	(aadd(::aObjTotaliz, Nil) , nObj++)
	@ 15, (nPxlRight*0.33)+75 SAY ::aObjTotaliz[nObj] VAR ::nVlTotE1Sel SIZE 50, 9 OF oPnl PICTURE PesqPict("SE1","E1_VALOR") RIGHT COLOR CLR_BLUE PIXEL

	TSay():New( 25, nPxlRight*0.33, {|| "Valor Total Títulos Inconstentes:" }, oPnl,,,,,,.T.,CLR_BLACK,,150,9 )
	(aadd(::aObjTotaliz, Nil) , nObj++)
	@ 25, (nPxlRight*0.33)+75 SAY ::aObjTotaliz[nObj] VAR ::nVlTotE1Inc SIZE 50, 9 OF oPnl PICTURE PesqPict("SE1","E1_VALOR") RIGHT COLOR CLR_BLUE PIXEL


	TSay():New( 05, nPxlRight*0.66, {|| "Valor Total Extrato Operadora:" }, oPnl,,,,,,.T.,CLR_BLACK,,150,9 )
	(aadd(::aObjTotaliz, Nil) , nObj++)
	@ 05, (nPxlRight*0.66)+75 SAY ::aObjTotaliz[nObj] VAR ::nVlTotExGer SIZE 50, 9 OF oPnl PICTURE PesqPict("SE1","E1_VALOR") RIGHT COLOR CLR_BLUE PIXEL

	TSay():New( 15, nPxlRight*0.66, {|| "Valor Total Extrato Selecionados:" }, oPnl,,,,,,.T.,CLR_BLACK,,150,9 )
	(aadd(::aObjTotaliz, Nil) , nObj++)
	@ 15, (nPxlRight*0.66)+75 SAY ::aObjTotaliz[nObj] VAR ::nVlTotExSel SIZE 50, 9 OF oPnl PICTURE PesqPict("SE1","E1_VALOR") RIGHT COLOR CLR_BLUE PIXEL

	TSay():New( 25, nPxlRight*0.66, {|| "Valor Total Extrato Inconstentes:" }, oPnl,,,,,,.T.,CLR_BLACK,,150,9 )
	(aadd(::aObjTotaliz, Nil) , nObj++)
	@ 25, (nPxlRight*0.66)+75 SAY ::aObjTotaliz[nObj] VAR ::nVlTotExInc SIZE 50, 9 OF oPnl PICTURE PesqPict("SE1","E1_VALOR") RIGHT COLOR CLR_BLUE PIXEL
...
METHOD DoNewGetSE1(oPnl) CLASS UT019ABA
	...
	::oMSNewGeE1:oBrowse:bHeaderClick := {|oBrw1,nCol| if(nCol > 1, UOrdGrid(@::oMSNewGeE1, @nCol), )}
...
Static Function AddCamposSE1(_cOperad, _cCodLay, _aHeader, _aEmpty, lPrincipal)
	...
	Local nX := 0
	Local aCposE1 := {"E1_PREFIXO","E1_NUM","E1_PARCELA","E1_TIPO","E1_EMISSAO","E1_VENCTO","E1_VLRREAL","E1_VALOR","E1_ACRESC","E1_DECRESC","E1_CLIENTE","E1_LOJA","E1_NOMCLI"}
	Local aHeadTmp
	Local cTipoCp := ""
...
METHOD DoNewGetExt(oPnl) CLASS UT019ABA
	...
	::oMSNewGeEx:oBrowse:bHeaderClick := {|oBrw1,nCol| if(nCol > 1, UOrdGrid(@::oMSNewGeEx, @nCol), )}
		...
		while U99->(!Eof()) .AND. U99->U99_FILIAL+U99->U99_OPERAD+U99->U99_CODIGO == xFilial("U99")+cOperad+_cCodLay
			...
			if U99->U99_UTILIZ == "C" //comparação pega da X3
				if lAddComp
					...
					if Valtype(cTipoCp)=="C" //se campo existe
						aHeadTmp := UAHEADER(U99->U99_CAMPO)
...
METHOD DoRelaciona(nRecE1,nRecEx) CLASS UT019ABA
	...
	Local nPosCpEx := 0
	Local nPosVlrE := 0
	Local lAjSE1Ext := SuperGetMv("TP_AJE1EXT",,.F.) //define se ajusta campos NSU e AUT da SE1 conforme extrato
	Default nRecE1 := 0
	Default nRecEx := 0
...
METHOD DoMsGetSE1() CLASS UT019ABA
	...
	::oMsNewSE1:oBrowse:bHeaderClick := {|oBrw,nCol,aDim| if(::oMsNewSE1:oBrowse:nColPos<>111 .and. nCol == 1,(::DoMarcaTodos(::oMsNewSE1, 2, "BR_VERDE"),oBrw:SetFocus()), if(nCol > 1, UOrdGrid(@::oMsNewSE1, @nCol), ))}
	...
	If MsgYesNo("Haverá o faturamento dos registros selecionados, separando por bandeira. Deseja continuar?","Atençao")
		...
		for nX := 1 to len(aFatBand)
			...
			For nY := 1 to len(aFatBand[nX][2])
				...
				aAdd(aReg,{.T.,; //selecionado
	    			oLeg,; //legenda
	    			SE1->E1_FILIAL,; //filial
	    			SE1->E1_TIPO,; //tipo
					"",; //origem
	    			"",; //descrçao forma
	    			...
	    			"",; //natureza
	    			SE1->E1_PORTADO,; //portador
					SE1->E1_AGEDEP,; //agencia
					SE1->E1_CONTA,; //conta
					"",; //nome banco
	    			"",; //placa
	    			SE1->E1_CLIENTE,; // cliente
	    			SE1->E1_LOJA,; //loja
	    			SE1->E1_NOMCLI,; //nome cli
					"",; //classe
					"",; //descriçao condição
	    			DToC(SE1->E1_EMISSAO),; //data emissao
	    			DToC(SE1->E1_VENCTO),; //vencimento
	    			Transform(SE1->E1_VALOR,"@E 9,999,999,999,999.99"),; //valor
	    			Transform(SE1->E1_SALDO,"@E 9,999,999,999,999.99"),; //saldo
	    			Transform(SE1->E1_DESCONT,"@E 9,999,999,999,999.99"),; //desconto
	    			Transform(SE1->E1_MULTA,"@E 9,999,999,999,999.99"),; //multa
	    			Transform(SE1->E1_JUROS,"@E 9,999,999,999,999.99"),; //juros
	    			Transform(SE1->E1_ACRESC,"@E 9,999,999,999,999.99"),; //acrescimo
	    			Transform(SE1->E1_DECRESC,"@E 9,999,999,999,999.99"),; //decrescimo
					FValAcess(SE1->E1_PREFIXO,SE1->E1_NUM,SE1->E1_PARCELA,SE1->E1_TIPO,SE1->E1_CLIENTE,SE1->E1_LOJA,SE1->E1_NATUREZ, Iif(Empty(SE1->E1_BAIXA),.F.,.T.),"","R",dDataBase,,SE1->E1_MOEDA,1,SE1->E1_TXMOEDA),;//valor acess
	    			space(9),; //num fatura
	    			SE1->(Recno()),; //recno
					"",; //motivo saque
					"",; //produto OS
			...
			if len(aReg) > 0
				...
				if ::DoTelaAuxFat(aFatBand[nX][3]) //abre tela dados complementares
					...
					if SA1->(DbSeek(xFilial("SA1")+aFatBand[nX][1]+"01" ))
					    ...
					    aRetFat := U_RUT034GV(,aReg,SA1->A1_COD,SA1->A1_LOJA,3,,,,,,,,aCpoComp, lFatFat, ::aValAcess)
...
METHOD DoNewGetVlA(oPnl) CLASS UT019ABA
	...
	cQry += "	AND FKC_RECPAG IN ('1','3') " //carteira receber ou ambas
...
//---------------------------------------------------------------
METHOD DoQryTitulos(_dDtEmis1, _dDtEmis2, _cBandeira, _dDtVenc1, _dDtVenc2) CLASS UT019ABA
	...
	if !empty(cAdmFin)
		...
		if !Empty(_dDtEmis1) .AND. !Empty(_dDtEmis2)
			cQry += " 	AND E1_EMISSAO BETWEEN '"+dtos(_dDtEmis1)+"' AND '"+dtos(_dDtEmis2)+"' "
		elseif !Empty(_dDtEmis1) //preencheu só data de
			cQry += " 	AND E1_EMISSAO >= '"+dtos(_dDtEmis1)+"' "
		elseif !Empty(_dDtEmis2) //preencheu só data ate
			cQry += " 	AND E1_EMISSAO <= '"+dtos(_dDtEmis2)+"' "
		endif
		if !Empty(_dDtVenc1) .AND. !Empty(_dDtVenc2)
			cQry += " 	AND E1_VENCTO BETWEEN '"+dtos(_dDtVenc1)+"' AND '"+dtos(_dDtVenc2)+"' "
		elseif !Empty(_dDtVenc1) //preencheu só data de
			cQry += " 	AND E1_VENCTO >= '"+dtos(_dDtVenc1)+"' "
		elseif !Empty(_dDtVenc2) //preencheu só data ate
			cQry += " 	AND E1_VENCTO <= '"+dtos(_dDtVenc2)+"' "
		endif
...
METHOD DoImpExtrato() CLASS UT019ABA
	...
	//aFiles={{_cArquivo, _dDtEmis1, _dDtEmis2, _lConsCab, _cBandeira, _cNmBand, _dDtVenc1, _dDtVenc2}...}
	For nX := 1 to len(::aFiles) //todos os arquivos
		...
		if lRet
			...
			if lRet
				::DoQryTitulos(::aFiles[nX][2], ::aFiles[nX][3], ::aFiles[nX][5], ::aFiles[nX][7], ::aFiles[nX][8]) //faz busca de títulos
			endif
		endif
...
//---------------------------------------------------------------
// faz importação do extrato da operadora
// aFile = {_cArquivo, _dDtEmis1, _dDtEmis2, _lConsCab, _cBandeira, _cNmBand, _dDtVenc1, _dDtVenc2}
//---------------------------------------------------------------
		...
		if U99->(DbSeek(xFilial("U99")+::cOperad+::cCodLay))
			while U99->(!Eof()) .AND. U99->U99_FILIAL+U99->U99_OPERAD+U99->U99_CODIGO == xFilial("U99")+::cOperad+::cCodLay
				...
				if ((U99->U99_POSINI+U99->U99_TAMANH) <= 0 .OR. (U99->U99_POSINI+U99->U99_TAMANH-1) > len(cLinha) )
					...
					//Conout("#RUTIL034 TXT: U99->U99_POSINI+U99->U99_TAMANH = "+STR(U99->U99_POSINI+U99->U99_TAMANH) + " #### len(cLinha) = " + Str(len(cLinha)))
					//lRet := .F.
					lLinNotImp := .T.
					EXIT
				endif
		...
		if U99->(DbSeek(xFilial("U99")+::cOperad+::cCodLay))
			while U99->(!Eof()) .AND. U99->U99_FILIAL+U99->U99_OPERAD+U99->U99_CODIGO == xFilial("U99")+::cOperad+::cCodLay
				...
				if (empty(U99->U99_COLUNA) .OR. val(U99->U99_COLUNA) <= 0 .OR. val(U99->U99_COLUNA) > len(aLinha) )
					//Conout("#RUTIL034 CSV: Linha ignorada -> U99_COLUNA = "+U99->U99_COLUNA + " #### len(aLinha) = " + Str(len(aLinha)))
					lLinNotImp := .T.
					EXIT
				endif
	...
	For nLoop := 1 to 3 //faz tres tentativas para abrir arquivo
		if oXls2Csv:Import() //processa importação dos dados, gerando arquivo CSV
			...
			if len(aDados) > 0
					...
					if U99->(DbSeek(xFilial("U99")+::cOperad+::cCodLay))
						while U99->(!Eof()) .AND. U99->U99_FILIAL+U99->U99_OPERAD+U99->U99_CODIGO == xFilial("U99")+::cOperad+::cCodLay
							...
							if nColXls > len(aDados[nX])
								//Conout("#RUTIL034 XLS Ignorada: nColXls = "+STR(nColXls) + " #### len(aDados[nX]) = " + Str(len(aDados[nX])))
								lLinNotImp := .T.
								EXIT
							endif
				...
				//Conout("#RUTIL034 XLS *2: nLoop = "+STR(nLoop) )
				if nLoop == 3
...
METHOD DoAtuTotal() CLASS UT019ABA
	...
	nPosTotTit := aScan(::aHeader,{|x| AllTrim(x[2]) == alltrim(::cCpVlrEx) })
    ...
    For nX := 1 to len(::aDados)
    	if ::aDados[nX][len(::aHeader)+1] > 0
	    	...
	    	::nVlTotE1Ger += ::aDados[nX][nPosTotTit] + ::aDados[nX][nPosAcres] - ::aDados[nX][nPosDecres]
            If nPosValE>0
	    	...
	    	if ::aDados[nX][1] == "LBOK"
	    		::nQtdSel++
	    		::nVlTotE1Sel += ::aDados[nX][nPosTotTit] + ::aDados[nX][nPosAcres] - ::aDados[nX][nPosDecres]
	    		If nPosValE>0
    ...
    nPosValT := aScan(::aHeaderSE1,{|x| AllTrim(x[2]) == alltrim(::cCpVlrEx) })
	nPosValE := aScan(::aHeaderExt,{|x| AllTrim(x[2]) == alltrim(::cCpVlrEx) })
    For nX := 1 to len(::aDadosSE1)
...
if cNum <> Nil .AND. !empty(cNum)
	...
	cPref := SuperGetMV("MV_XPFXFCA",,"REC") //prefixo da fatura de cartao
...
//-------------------------------------------------------------
// Monta string de filtro para comparação SE1 x item extrato
//-------------------------------------------------------------
	...
	for nY := 1 to len(aExtrato[nX])
		if aExtrato[nX][nY][3] .AND. (aExtrato[nX][nY][4] == cNivel .OR. empty(cNivel)) //se é campo de comparação
			...
			if valtype(aExtrato[nX][nY][2])=="C" //caractere
				...
				If aExtrato[nX][nY][1] $ "E1_VLRREAL/E1_VALOR"
...
/*/
Static Function UOrdGrid(oObj,nColum) //UOrdGrid(oNewGet,nColuna)
...
05 - Decimal
06 - Validação
07 - Reservado - X3_USADO
08 - Tipo
09 - Consulta F3
10 - Reservado - X3_CONTEXT
11 - ComboBox - X3_CBOX
12 - Reservado - X3_RELACAO
@author  author
@since   date
@version version
/*/
//-------------------------------------------------------------------
Static Function UAHEADER(cField)
...
/**************************************************************************************************************************************************/
User Function RUT034GV(_aHeader,_aReg,cCliFat,cLojaFat,nOpcAuto,cNumLiqCan,dDtVencRen,nAcresRen,nDecresRen,aParcelas,;
...
Default _aHeader	:= {"","","Filial","Tipo","Origem Fatura","Descrição","Prefixo","Número","Parcela","Natureza","Portador","Depositaria","Num da Conta",;
						"Nome Banco","Placa","Cliente","Loja","Nome","Classe","Cond. pagto.","Dt. Emissão","Dt. Vencimento","Valor","Saldo",;
						"Desconto","Multa","Juros","Acréscimo","Decréscimo","Vlr Acessorios","Fatura","R_E_C_N_O_","Mot. Saque","Prod. OS","Num.Carta F.","Cod.Mutuo"}

Private aVlrAcess := _aVlrAcess

nPosFilial		:= Ascan(_aHeader,{|x| AllTrim(x) == "Filial"})
nPosTipo 		:= Ascan(_aHeader,{|x| AllTrim(x) == "Tipo"})
nPosOriFat		:= Ascan(_aHeader,{|x| AllTrim(x) == "Origem Fatura"})
nPosDescri		:= Ascan(_aHeader,{|x| AllTrim(x) == "Descrição"})
nPosPrefixo		:= Ascan(_aHeader,{|x| AllTrim(x) == "Prefixo"})
nPosNumero		:= Ascan(_aHeader,{|x| AllTrim(x) == "Número"})
nPosParcela		:= Ascan(_aHeader,{|x| AllTrim(x) == "Parcela"})
nPosNaturez		:= Ascan(_aHeader,{|x| AllTrim(x) == "Natureza"})
nPosPortado		:= Ascan(_aHeader,{|x| AllTrim(x) == "Portador"})
nPosDeposit		:= Ascan(_aHeader,{|x| AllTrim(x) == "Depositaria"})
nPosNConta		:= Ascan(_aHeader,{|x| AllTrim(x) == "Num da Conta"})
nPosBanco		:= Ascan(_aHeader,{|x| AllTrim(x) == "Nome Banco"})
nPosPlaca		:= Ascan(_aHeader,{|x| AllTrim(x) == "Placa"})
nPosCliente		:= Ascan(_aHeader,{|x| AllTrim(x) == "Cliente"})
nPosLoja		:= Ascan(_aHeader,{|x| AllTrim(x) == "Loja"})
nPosNome		:= Ascan(_aHeader,{|x| AllTrim(x) == "Nome"})
nPosClasse		:= Ascan(_aHeader,{|x| AllTrim(x) == "Classe"})
nPosCondPg		:= Ascan(_aHeader,{|x| AllTrim(x) == "Cond. pagto."})
nPosEmissao		:= Ascan(_aHeader,{|x| AllTrim(x) == "Dt. Emissão"})
nPosVencto		:= Ascan(_aHeader,{|x| AllTrim(x) == "Dt. Vencimento"})
nPosValor		:= Ascan(_aHeader,{|x| AllTrim(x) == "Valor"})
nPosSaldo		:= Ascan(_aHeader,{|x| AllTrim(x) == "Saldo"})
nPosDescont		:= Ascan(_aHeader,{|x| AllTrim(x) == "Desconto"})
nPosMulta		:= Ascan(_aHeader,{|x| AllTrim(x) == "Multa"})
nPosJuros		:= Ascan(_aHeader,{|x| AllTrim(x) == "Juros"})
nPosAcresc		:= Ascan(_aHeader,{|x| AllTrim(x) == "Acréscimo"})
nPosDecres		:= Ascan(_aHeader,{|x| AllTrim(x) == "Decréscimo"})
nPosVlAcess		:= Ascan(_aHeader,{|x| AllTrim(x) == "Vlr Acessorios"})
nPosFatura		:= Ascan(_aHeader,{|x| AllTrim(x) == "Fatura"})
nPosRecno		:= Ascan(_aHeader,{|x| AllTrim(x) == "R_E_C_N_O_"})
nPosMotiv		:= Ascan(_aHeader,{|x| AllTrim(x) == "Mot. Saque"})
nPosProdOs		:= Ascan(_aHeader,{|x| AllTrim(x) == "Prod. OS"})
nPosNCFret		:= Ascan(_aHeader,{|x| AllTrim(x) == "Num.Carta F."})
nPosNMutuo		:= Ascan(_aHeader,{|x| AllTrim(x) == "Cod.Mutuo"})
	...
	For nI := 1 To Len(_aReg)
		...
		if Empty(cNat)
			if Alltrim(SE1->E1_TIPO) == "CC"
				cNat := SuperGetMv( "MV_XNFATCC" , .F. , "CARTAO",)
			elseif Alltrim(SE1->E1_TIPO) == "CD"
				cNat := SuperGetMv( "MV_XNFATCD" , .F. , "TEF",)
			else
...
Else //Cancelamento 
	...
	//Tratametno provisório, para caso de E1_NUMLIQ repetida em outra filial
	...
	//independente se deu certo ou não, volto
	aAreaSE1 := SE1->(GetArea())
