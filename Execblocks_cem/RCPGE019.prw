...
if nMark > 0
	...
	For nX := 1 To Len(oGrid:aCols)
		...
		if oGrid:aCols[nX,nPosMark] == "CHECKED"
			...
			if U41->(MsSeek(xFilial("U41")+cContrato+cItem)) 
				...
				lRet := .F.
				MsgInfo("Não encontrado a Retirada de Cinzas no histórico de Retiradas Item ("+Alltrim(cItem)+"), Favor verifique a retirada realizada")
			endif
	...
	if Len(aImpRecibos)
		...
		lRet := .F.
		Help(,,'Help',,"Não Existem itens a serem Impressos, Favor verifique os apontamentos de retirada de cinzas!",1,0)
	endif
